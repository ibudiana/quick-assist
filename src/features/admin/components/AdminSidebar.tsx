...
import { useTheme } from "@/features/core/components/ThemeProvider";
...
export function AdminSidebar({
  ...
  return (
    <>
      ...
      <div
        className={`w-16 bg-gray-900 flex flex-col items-center py-6 shrink-0 z-50 justify-between h-full fixed md:relative transition-transform duration-300 ${isMobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        <div className="flex flex-col items-center w-full relative">
          {/* Mobile close button */}
          <button
            onClick={closeMobile}
            className="absolute -right-10 top-2 p-2 bg-gray-900 text-white rounded-r-xl md:hidden"
          >
            <FiX size={20} />
          </button>
